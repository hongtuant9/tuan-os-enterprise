...
function whatsappPageEntity(): "tce" | "lavender" | "ruby" | "cozy" | "unknown" {
  const value = process.env.WHATSAPP_PAGE_ENTITY?.trim().toLowerCase();
...
export async function POST(request: Request) {
  try { assertCustomerChannelReceiveEnabled("whatsapp"); } catch { return NextResponse.json({ error: "WhatsApp receive path closed" }, { status: 423 }); }
