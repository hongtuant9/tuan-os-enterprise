...
import { getZaloAppId, ZaloOAuthService } from "@/server/integrations/zalo/oauth-service";
...
function bridgePolicyEnabled() {
  return process.env.TCE_ZALO_BRIDGE_ENABLED?.trim().toLowerCase() !== "false";
}
...
function verifySignature(
  ...
  // Zalo OA docs: sha256(appId + raw JSON data + timestamp + OA secret key).
...
export async function POST(req: NextRequest) {
  if (!bridgePolicyEnabled()) {
    return NextResponse.json({ ok: false, error: "zalo_bridge_disabled" }, { status: 503 });
  }

  const oauth = new ZaloOAuthService();
  const secret = await oauth.getWebhookSecret();
  if (!secret) {
  ...
  if (!verifySignature(rawBody, req, payload, getZaloAppId(), secret)) {
...
export async function GET() {
  const status = await new ZaloOAuthService().connectionStatus();
  return NextResponse.json({
    ok: true,
    service: "zalo-operations-bridge",
    bridgeEnabled: bridgePolicyEnabled() && status.configured,
    signatureConfigured: status.configured,
    oauthConnected: status.connected,
    oaIdSet: status.oaIdSet,
    mode: "shadow",
