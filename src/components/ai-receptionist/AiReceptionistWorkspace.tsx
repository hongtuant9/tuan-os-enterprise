...
function BookingDraftLab({ conversations, canManage }: { conversations: ReceptionistConversation[]; canManage: boolean }) {
  ...
  const [roomOptions, setRoomOptions] = useState<Array<{id:string;code:string;name:string;available:number;version:number}>>([]);
  ...
  const [feedback, setFeedback] = useState(""); const [pending, startTransition] = useTransition();
  ...
  function createDraft() { setFeedback(""); if (!selectedRoom) return setFeedback("Hãy tải và chọn hạng phòng live từ KiotViet."); startTransition(async () => { const result = await prepareBookingDraftAction({ conversationId, propertyId: selectedConversation?.propertyId ?? null, guestName, guestContact, checkIn, checkOut, adults, children, roomCount, roomClassId: selectedRoom.id, roomClassName: selectedRoom.name, quotedPrice: quotedPrice ? Number(quotedPrice) : null, priceSource: priceSource || null }); if (!result.ok) return setFeedback(result.error); setFeedback(result.data?.duplicate ? "Booking draft trùng đã tồn tại; không tạo bản ghi mới." : "Đã tạo booking draft nội bộ. Chưa ghi KiotViet, chưa gửi khách."); router.refresh(); }); }
  return <div className="mb-6 rounded-xl border border-[var(--accent)]/25 bg-[var(--surface)] p-5"><div className="flex items-start justify-between gap-3"><div><h3 className="text-base font-semibold text-[var(--ink-primary)]">Booking Draft Lab</h3><p className="mt-1 text-sm text-[var(--ink-muted)]">Manager-only · KiotViet read-only · draft CRM nội bộ · chưa gửi confirmation.</p></div><Pill label="WRITE OFF" tone="warn" /></div>
    ...
    <div className="mt-3 flex flex-wrap gap-2"><button type="button" disabled={!canManage||pending||!checkIn||!checkOut} onClick={loadRooms} className="rounded-lg border border-[var(--accent)]/40 px-3 py-2 text-sm font-medium text-[var(--accent)] disabled:opacity-40">Đọc phòng trống KiotViet</button><select value={roomClassId} onChange={(e)=>setRoomClassId(e.target.value)} className="min-w-72 rounded-lg border border-[var(--border-hairline)] bg-[var(--page)] px-3 py-2 text-sm"><option value="">Chọn hạng phòng live</option>{roomOptions.map((r)=><option key={r.id} value={r.id}>{r.name} · còn {r.available}</option>)}</select><input type="number" min="1" value={roomCount} onChange={(e)=>setRoomCount(Number(e.target.value))} className="w-24 rounded-lg border border-[var(--border-hairline)] bg-[var(--page)] px-3 py-2 text-sm"/><input type="number" min="1" value={adults} onChange={(e)=>setAdults(Number(e.target.value))} className="w-24 rounded-lg border border-[var(--border-hairline)] bg-[var(--page)] px-3 py-2 text-sm"/><input type="number" min="0" value={children} onChange={(e)=>setChildren(Number(e.target.value))} className="w-24 rounded-lg border border-[var(--border-hairline)] bg-[var(--page)] px-3 py-2 text-sm"/></div>
    <div className="mt-3 grid gap-3 md:grid-cols-2"><input value={quotedPrice} onChange={(e)=>setQuotedPrice(e.target.value)} placeholder="Giá VERIFIED (để trống nếu chưa có)" className="rounded-lg border border-[var(--border-hairline)] bg-[var(--page)] px-3 py-2 text-sm"/><input value={priceSource} onChange={(e)=>setPriceSource(e.target.value)} placeholder="Nguồn giá VERIFIED, ví dụ Master Sheet 03_GIA..." className="rounded-lg border border-[var(--border-hairline)] bg-[var(--page)] px-3 py-2 text-sm"/></div>
    <button type="button" disabled={!canManage||pending||!conversationId||!guestName||!selectedRoom} onClick={createDraft} className="mt-4 rounded-lg bg-[var(--accent)] px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-40">{pending?"Đang xử lý...":"Tạo booking draft nội bộ"}</button>{feedback&&<p className="mt-3 text-sm text-[var(--ink-secondary)]">{feedback}</p>}</div>;
}
