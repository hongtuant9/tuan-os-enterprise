  ...
  return (
    <aside className="hidden w-64 shrink-0 flex-col border-r border-[var(--border-hairline)] bg-[var(--surface)] px-4 py-6 md:flex">
      <Link href="/" className="mb-6 flex items-center gap-2 px-2">
        <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-[var(--accent)] text-sm font-bold text-white">T</div>
        <div>
          <p className="text-sm font-semibold text-[var(--ink-primary)]">TUAN OS</p>
          <p className="text-xs text-[var(--ink-muted)]">Hospitality Control Center</p>
        </div>
      </Link>
      ...
      <div className="mt-auto flex items-center justify-between px-2 pt-6 text-xs text-[var(--ink-muted)]">
        ...
        <form action={signOut}>
          <button type="submit" className="text-[var(--ink-secondary)] transition-colors hover:text-[var(--ink-primary)]">Đăng xuất</button>
        </form>
      </div>
    </aside>
  );
