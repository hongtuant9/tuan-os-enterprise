  ...
  return (
    <div className="mt-4 grid gap-3 rounded-xl border border-amber-500/20 bg-amber-500/[0.04] p-4 text-xs md:grid-cols-2">
      ...
      <div>
        ...
        <p className="mt-1 text-[var(--ink-secondary)]">{approval.sourceChannel || "AI Channel Auditor"}</p>
    ...
    <section id="approval-queue" className="mb-10 scroll-mt-6">
      ...
      <div className="flex flex-col gap-3">
        {approvals.map((approval) => {
          ...
          return (
            <div key={approval.id} className="rounded-xl border border-[var(--border-hairline)] bg-[var(--surface)] p-5">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                <div className="min-w-0">
                  ...
                  <p className="mt-1 text-xs text-[var(--ink-muted)]">{approval.unit} · {approval.requestedBy} · {formatSubmittedAt(approval.submittedAt)}</p>
                </div>
                <div className="flex shrink-0 gap-2">
                  <button type="button" disabled={approval.status !== "pending" || busy} onClick={() => decide(approval.id, "approved")} className="rounded-lg bg-[var(--status-good)] px-3 py-1.5 text-xs font-medium text-white transition-opacity hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-40">
                    {busy ? "Đang xử lý..." : "Duyệt"}
                  </button>
                  <button type="button" disabled={approval.status !== "pending" || busy} onClick={() => decide(approval.id, "rejected")} className="rounded-lg border border-[var(--border-hairline)] px-3 py-1.5 text-xs font-medium text-[var(--ink-secondary)] transition-colors hover:border-[var(--status-bad)]/50 hover:text-[var(--status-bad)] disabled:cursor-not-allowed disabled:opacity-40">
                    {busy ? "Đang xử lý..." : "Từ chối"}
                  </button>
                </div>
              </div>
              <MasterChangeDetails approval={approval} />
              {errorById[approval.id] ? <p className="mt-3 rounded-lg border border-rose-500/20 bg-rose-500/[0.08] p-3 text-xs text-rose-200">{errorById[approval.id]}</p> : null}
            </div>
          );
        })}
      </div>
