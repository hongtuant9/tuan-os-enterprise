...
export default async function CustomerDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="flex min-h-screen bg-[var(--page)]"><Sidebar/><main className="min-w-0 flex-1 px-4 py-6 md:px-8 md:py-8 xl:px-10">
    <div className="mb-5"><Link href="/customers" className="text-sm text-[var(--ink-muted)] hover:underline">← Khách hàng</Link><div className="mt-3 flex flex-wrap items-center gap-3"><h1 className="text-2xl font-semibold text-[var(--ink-primary)]">{customer.displayName}</h1><span className="rounded-full border border-[var(--border-hairline)] px-3 py-1 text-xs font-semibold">{customer.journeyStage}</span></div><p className="mt-2 text-sm text-[var(--ink-muted)]">Kênh: {customer.channels.join(", ") || "—"} · Acquisition: {customer.acquisitionSources.join(", ") || "—"} · Journey: {customer.journeyEntries.join(", ") || "GENERAL"}</p></div>
    <div className="mb-6 grid grid-cols-2 gap-3 md:grid-cols-4"><Card label="Hội thoại" value={customer.conversationCount}/><Card label="Booking" value={`${customer.verifiedBookingCount}/${customer.bookingCount}`}/><Card label="Upsell events" value={customer.upsellEventCount}/><Card label="Last seen" value={new Date(customer.lastSeenAt).toLocaleDateString("vi-VN")}/></div>
